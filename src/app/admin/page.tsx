...
export default async function AdminOverviewPage() {
  const [
    ...
    prisma.profile.findMany({
      ...
      take: 5,
      select: { username: true, displayName: true, createdAt: true, membershipTier: true },
    }),
  ...
  const stats = [
    ...
    { label: "Kommentarer", value: totalComments, icon: MessageSquare, color: "text-purple-600 bg-purple-50" },
    { label: "Veckotillväxt", value: `${((newUsersThisWeek / Math.max(totalUsers, 1)) * 100).toFixed(1)}%`, icon: TrendingUp, color: "text-amber-600 bg-amber-50" },
  ];
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">
          Välkommen till Minodlings adminpanel
        </p>
      </div>
              ...
              <div key={u.username} className="flex items-center justify-between px-5 py-3">
                ...
                <div className="text-right">
                  {u.membershipTier === "PREMIUM" && (
                    <span className="inline-flex text-xs font-medium bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">
                      Premium
                    </span>
                  )}
