...
import {
  Menu, X, Sprout, Bell, ChevronDown, ChevronLeft,
  LayoutDashboard, BookOpen, AlarmClock,
  User, Settings, ShieldCheck, LogOut, Leaf, ShoppingBag,
} from "lucide-react";
...
export function ShopNavbar({ user, navItems }: ShopNavbarProps) {
    ...
    <header className="sticky top-0 z-50 w-full border-b border-gray-100 bg-white/95 backdrop-blur-sm">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-14 items-center gap-3">
          ...
          <Link
            href="/"
            className="hidden sm:flex items-center gap-1 text-[12px] font-medium text-gray-400 hover:text-gray-600 transition-colors shrink-0 mr-1"
          >
          ...
          >
            ...
            <span className="hidden sm:flex items-center gap-1.5 font-bold text-[15px] text-gray-900 tracking-tight">
              min<span className="text-green-600">odling</span>
              <span className="text-[11px] font-semibold text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded-md">Butik</span>
            </span>
          </Link>

          {/* ── Kategori-nav (desktop) ── */}
          {navItems.length > 0 && (
            <nav className="hidden lg:flex items-center gap-0.5 flex-1 overflow-x-auto ml-2">
              {navItems.map((item) => (
                <Link
                  key={item.id}
                  href={item.href}
          ...
          <div className="flex items-center gap-1">
            {user ? (
              <>
                <Link
                  href="/paminnelser"
                  className="h-8 w-8 flex items-center justify-center rounded-lg text-gray-500 hover:bg-gray-50 hover:text-gray-700 transition-colors"
      ...
      {mobileOpen && (
        <div className="lg:hidden border-t border-gray-100 bg-white">
          <div className="px-4 py-3 space-y-0.5">
            {/* Tillbaka till startsidan (mobil) */}
            <Link
              href="/"
              onClick={() => setMobileOpen(false)}
              className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-400 hover:bg-gray-50 transition-colors"
            >
              <ChevronLeft className="h-4 w-4" />
              Tillbaka till startsidan
            </Link>

            {/* Alla kategorier i butiken */}
            <Link
              href="/butik"
              onClick={() => setMobileOpen(false)}
              className={cn(
                "flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors",
                pathname === "/butik" ? "bg-green-50 text-green-700" : "text-gray-700 hover:bg-gray-50"
              )}
            >
